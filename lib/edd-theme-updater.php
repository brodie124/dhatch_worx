...
define( 'TT_EDD_SL_STORE_URL', 'http://themetrail.com' ); // add your own unique prefix to prevent conflicts

// The name of your product. This should match the download name in EDD exactly
define( 'TT_WORX_EDD_SL_THEME_NAME', 'Worx - WordPress Portfolio Theme (Pro)' ); // add your own unique prefix to prevent conflicts
...
if ( !class_exists( 'EDD_SL_Theme_Updater' ) ) {
	...
	include( TT_LIB . '/EDD_SL_Theme_Updater.php' );
}
...
function edd_sl_sample_theme_updater() {
	...
	$edd_updater = new EDD_SL_Theme_Updater( array(
			'remote_api_url' 	=> TT_EDD_SL_STORE_URL, 			// Our store URL that is running EDD
			...
			'item_name' 			=> TT_WORX_EDD_SL_THEME_NAME,	// The name of this theme
			'author'					=> 'ThemeTrail'								// The author's name
		)
	);
}
...
function edd_sample_theme_license_menu() {
	add_theme_page( 'Theme License', 'Theme License', 'manage_options', 'worx-license', 'edd_sample_theme_license_page' );
}
...
function edd_sample_theme_activate_license() {
	...
	if( isset( $_POST['edd_theme_license_activate'] ) ) {
		...
		$api_params = array(
			...
			'license' => $license,
			'item_name' => urlencode( TT_WORX_EDD_SL_THEME_NAME )
		);

		$response = wp_remote_get( add_query_arg( $api_params, TT_EDD_SL_STORE_URL ), array( 'timeout' => 15, 'sslverify' => false ) );
...
function edd_sample_theme_deactivate_license() {
	...
	if( isset( $_POST['edd_theme_license_deactivate'] ) ) {
		...
		$api_params = array(
			...
			'license' 	=> $license,
			'item_name' => urlencode( TT_WORX_EDD_SL_THEME_NAME ) // the name of our product in EDD
		);
